...
export function homePage({ origin, version, books, totals, limits, badjson, sse, range, etag, cookies, auth, compress, crosshost, x402, authWitness }) {
  const o = origin;
...
${endpoint({
  ...
  note: `Checked live against eight real clients on ${esc(authWitness.measured.slice(0, 10))} — ${authWitness.observations} observations, every one a row of <a href="/clients.jsonl"><code>/clients.jsonl</code></a> (family <code>auth</code>) and summarized in the <a href="/auth">index</a> under <code>witness</code>; a <em>dated capture</em>, not a live measurement, and every sentence that follows is computed from those rows rather than typed here. Each client was handed the documented fake credentials through its own mechanism for the flavor's scheme (<code>mechanism_kind</code> on every row says which), and every row records each request the client sent, its status, and whether it carried credentials. ${authWitness.findings.map(esc).join(' ')} None of this is a scoreboard: RFC 9110 §11 leaves preemptive sending, retry counts and the choice among challenges to the client, so these are descriptions of what a caller received, not conformance results.`,
})}
...
<p>And what real clients <em>do</em> with it is available as data too: <a href="/clients.jsonl"><code>/clients.jsonl</code></a> is one row per observation, across three families so far — six real HTTP clients and two non-decoding controls against all 21 <code>/compress</code> flavors (captured 2026-09-02), eight real clients started at every one of the nine <code>/crosshost</code> flavors — eight boundaries and a same-origin control — (captured 2026-09-17), and the same eight clients handed the documented fake credentials through their own mechanism at each of the 18 <code>/auth</code> flavors (captured 2026-09-18). Each row carries the client's version, what it received and what it reported: for <code>/compress</code>, whether the bytes were the documented plaintext; for <code>/crosshost</code>, which of the headers it was sent arrived on the far side of the redirect, on which host, over which transport; for <code>/auth</code>, how the credentials were configured, how many requests the client made, and what the final response said. This is the only data here that badhttp did not write about itself, and the three paragraphs above about those clients are checkable rather than merely asserted. The <a href="/clients">index</a> counts how many distinct answers each flavor produced — the disagreement is the finding — and states plainly that an outcome describes what the caller received and is never a verdict on the client: on <code>undeclared</code> handing back the bytes unchanged is correct, on <code>truncated</code> the same outcome is the bug, whether a credential should cross a given redirect boundary is something RFC 9110 does not say, and a client that 401s on <code>/auth/digest</code> because it has no Digest mechanism is a capability, not a defect.</p>
